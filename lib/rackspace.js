...
function organize (type, name, callback) {
...
exports.addFile = function (type, name, stream, callback ) {
	...
	authenticate(function (err, res) {
		...
		organize(type, name, function (container, newName) {

			logger.log({'container': container, 'newName': newName}, 'info');

			var rStream = request.put({
				url: storageUrl + '/' + container + '/' + newName,
				headers: {
